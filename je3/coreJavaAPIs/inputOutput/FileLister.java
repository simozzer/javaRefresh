...
public class FileLister extends Frame implements ActionListener, ItemListener {
    ...
    public void itemStateChanged(ItemEvent e) {
        int i = list.getSelectedIndex() -1; // ignore 1st 'special' entry.
        if (i <0 ) return;
        String filename = files[i];
        File f = new File(currentDir,filename);
        if (!f.exists()) throw new IllegalArgumentException("FileLister: no such file or directory");
    ...
    public static void usage() {
        System.out.println("Usage: java FileLuster [directory_name} [-e file_extension]");
        System.exit(0);
    }
